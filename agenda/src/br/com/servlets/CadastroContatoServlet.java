...
public class CadastroContatoServlet extends HttpServlet {
	...
	public static List<Contato> usuarioslist = new ArrayList<>();
	...
	public static void attusuarioslistByUserEmail(String userEmail) {
		CadastroContatoServlet.usuarioslist = new ArrayList<>();
		for(Contato contato : CadastroContatoServlet.contatos) {
			if(contato.getUserEmail().equals(userEmail)) {
				CadastroContatoServlet.usuarioslist.add(contato);
			} else {
				System.out.println(contato.getUserEmail()+" é diferente de "+userEmail);
			}
	
		}
		System.out.println(userEmail+" listaUser atualizada.");
	}
